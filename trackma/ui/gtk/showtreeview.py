...
class ProgressCellRenderer(Gtk.CellRenderer):
    ...
    def do_render(self, cr, widget, background_area, cell_area, flags):
        ...
        cr.set_source_rgb(*self.__getColor(self.colors['progress_bg']))
        ...
        if self.subvalue:
            ...
            cr.set_source_rgb(*self.__getColor(self.colors['progress_sub_bg']))
        ...
        if self.value:
            if self.value >= self.total:
                ...
                cr.set_source_rgb(
                    *self.__getColor(self.colors['progress_complete']))
                cr.rectangle(x, y, w, h)
            else:
                mid = int(w / float(self.total) * self.value)
                # set_source_rgb(0.6, 0.7, 0.8)
                cr.set_source_rgb(*self.__getColor(self.colors['progress_fg']))
                cr.rectangle(x, y, mid, h)
            cr.fill()

        if self.eps:
            # set_source_rgb(0.4, 0.5, 0.6)
            cr.set_source_rgb(*self.__getColor(self.colors['progress_sub_fg']))
